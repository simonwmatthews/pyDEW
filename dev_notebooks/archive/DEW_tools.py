...
class fluid:
    ...
    def make_input(self,nxmods=[],fep=-12.6,ph=4.0,uebal='H+'):
        s = self.input_preamble(fep=fep,uebal=uebal)
        s = self.input_nxmod(s,nxmods)
        for i in range(self.system.n_basis_species-1):
            if self.system.basis_species_names_soln[i] not in ['H2O','H+']:
                ...
                s = self.input_basis_species_logact('H+',- ph ,s)
        s = self.input_end_file(s)
        # print(self.basis_species_molalities)
    ...
    def read_eq3(self):
        ...
        _summed = 0.0
        self.output_mol = []
        for x in self.eq3output.elemental_comp.molality:
            self.output_mol.append(float(x))
            _summed += float(x)
        self.output_mol = np.array(self.output_mol)/_summed

        # print(self.output_mol)
