...
static const char *identifier = "Tue Oct  4 11:42:39 2022";

static const double T_r = 298.15;
static const double P_r = 1.0;
static const double H_TrPr = 61165.472499999996;
static const double S_TrPr = 205.033;
static const double k0 = 23.10248;
static const double k1 = 804.888;
static const double k2 = 1762835.0;
static const double k3 = 0.0;
static const double k5 = -18172.92;
static const double k6 = 0.003;
static const double V_TrPr = 0.0;
static const double v1 = 0.0;
static const double v2 = 0.0;
static const double v3 = 0.0;
static const double v4 = 0.0;
...
static double dewminerals_g(double T, double P) {
    double result = 0.0;
    result += H_TrPr + 2*sqrt(T)*k1 + (1.0/3.0)*((T)*(T)*(T))*k6 + (1.0/2.0)*((T)*(T))*k5 + T*k0 - T*(S_TrPr + (1.0/2.0)*((T)*(T))*k6 + T*k5 - 1.0/2.0*((T_r)*(T_r))*k6 - T_r*k5 + k0*log(T) - k0*log(T_r) + (1.0/2.0)*k2/((T_r)*(T_r)) + (1.0/3.0)*k3/((T_r)*(T_r)*(T_r)) + 2*k1/sqrt(T_r) - 1.0/2.0*k2/((T)*(T)) - 1.0/3.0*k3/((T)*(T)*(T)) - 2*k1/sqrt(T)) - 2*sqrt(T_r)*k1 - 1.0/3.0*((T_r)*(T_r)*(T_r))*k6 - 1.0/2.0*((T_r)*(T_r))*k5 - T_r*k0 + k2/T_r + (1.0/2.0)*k3/((T_r)*(T_r)) - k2/T - 1.0/2.0*k3/((T)*(T));
...
static double dewminerals_dgdt(double T, double P) {
    double result = 0.0;
    result += -S_TrPr + (1.0/2.0)*((T)*(T))*k6 - T*(T*k6 + k5 + k0/T + k2/((T)*(T)*(T)) + k3/((T)*(T)*(T)*(T)) + k1/pow(T, 3.0/2.0)) + (1.0/2.0)*((T_r)*(T_r))*k6 + T_r*k5 - k0*log(T) + k0*log(T_r) + k0 - 1.0/2.0*k2/((T_r)*(T_r)) - 1.0/3.0*k3/((T_r)*(T_r)*(T_r)) - 2*k1/sqrt(T_r) + (3.0/2.0)*k2/((T)*(T)) + (4.0/3.0)*k3/((T)*(T)*(T)) + 3*k1/sqrt(T);
...
static double dewminerals_d2gdt2(double T, double P) {
    double result = 0.0;
    result += (1.0/2.0)*T*(-2*k6 + 2*k0/((T)*(T)) + 6*k2/((T)*(T)*(T)*(T)) + 8*k3/pow(T, 5) + 3*k1/pow(T, 5.0/2.0)) - k5 - 2*k0/T - 4*k2/((T)*(T)*(T)) - 5*k3/((T)*(T)*(T)*(T)) - 5.0/2.0*k1/pow(T, 3.0/2.0);
...
static double dewminerals_d3gdt3(double T, double P) {
    double result = 0.0;
    result += -1.0/4.0*T*(8*k0/((T)*(T)*(T)) + 48*k2/pow(T, 5) + 80*k3/pow(T, 6) + 15*k1/pow(T, 7.0/2.0)) - k6 + 3*k0/((T)*(T)) + 15*k2/((T)*(T)*(T)*(T)) + 24*k3/pow(T, 5) + (21.0/4.0)*k1/pow(T, 5.0/2.0);
    if(((l1) != 0.0) || ((l2) != 0.0) || ((k_lambda) != 0.0) || ((T_lambda_Pr) != 0.0) || ((T_lambda_ref) != 0.0)) {
        if((T > (T_lambda_ref + k_lambda*(P - P_r))) && (P > (P_r)) && (T <= (T_lambda_Pr + k_lambda*(P - P_r)))) {
...
const char *O2_G_dewminerals_identifier(void) {
    return identifier;
}

const char *O2_G_dewminerals_name(void) {
    return "O2_G";
}

const char *O2_G_dewminerals_formula(void) {
    return "O2";
}

const double O2_G_dewminerals_mw(void) {
    return 31.9988;
}
...
const double *O2_G_dewminerals_elements(void) {
    return elmformula;
}

double O2_G_dewminerals_g(double T, double P) {
    return dewminerals_g(T, P);
}

double O2_G_dewminerals_dgdt(double T, double P) {
    return dewminerals_dgdt(T, P);
}

double O2_G_dewminerals_dgdp(double T, double P) {
    return dewminerals_dgdp(T, P);
}

double O2_G_dewminerals_d2gdt2(double T, double P) {
    return dewminerals_d2gdt2(T, P);
}

double O2_G_dewminerals_d2gdtdp(double T, double P) {
    return dewminerals_d2gdtdp(T, P);
}

double O2_G_dewminerals_d2gdp2(double T, double P) {
    return dewminerals_d2gdp2(T, P);
}

double O2_G_dewminerals_d3gdt3(double T, double P) {
    return dewminerals_d3gdt3(T, P);
}

double O2_G_dewminerals_d3gdt2dp(double T, double P) {
    return dewminerals_d3gdt2dp(T, P);
}

double O2_G_dewminerals_d3gdtdp2(double T, double P) {
    return dewminerals_d3gdtdp2(T, P);
}

double O2_G_dewminerals_d3gdp3(double T, double P) {
    return dewminerals_d3gdp3(T, P);
}

double O2_G_dewminerals_s(double T, double P) {
    return dewminerals_s(T, P);
}

double O2_G_dewminerals_v(double T, double P) {
    return dewminerals_v(T, P);
}

double O2_G_dewminerals_cv(double T, double P) {
    return dewminerals_cv(T, P);
}

double O2_G_dewminerals_cp(double T, double P) {
    return dewminerals_cp(T, P);
}

double O2_G_dewminerals_dcpdt(double T, double P) {
    return dewminerals_dcpdt(T, P);
}

double O2_G_dewminerals_alpha(double T, double P) {
    return dewminerals_alpha(T, P);
}

double O2_G_dewminerals_beta(double T, double P) {
    return dewminerals_beta(T, P);
}

double O2_G_dewminerals_K(double T, double P) {
    return dewminerals_K(T, P);
}

double O2_G_dewminerals_Kp(double T, double P) {
